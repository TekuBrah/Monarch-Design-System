...
writeFileSync(resolve(root, 'src/tokens/index.ts'), [
  ...
  // Generated by build-gradients.mjs and build-shadows.mjs — the pipeline is
  // seven layers, not five, and neither of these is hand-authored. Re-exported
  // here so this script leaves a complete barrel no matter where it runs in the
  // order. build-gradients.mjs omits shadows, which is why it must not run last.
